...
**注:重要说明**：本项目为 Embassy 的 **学习研究项目**，fork 自 embassy-rs/embassy，目标是系统性分析和理解 Embassy 的架构设计与实现细节。**不做开发，只做分析学习**。
...
| 目录 | 用途 | 查询方式 |
|------|------|----------|
| `docs/README.md` | 文档体系总览与目录 | 直接阅读 |
| `docs/01-overview.md` ~ `docs/27-patterns.md` | 分模块分析文档 | `ls docs/*.md` |
| `docs/appendix-*.md` | 附录（术语表、参考资料等） | `ls docs/appendix-*.md` |
...
| 开始学习分析 | tasks.md → docs/README.md → 目标模块源码 | docs/*.md |
| 深入模块分析 | codegraph_explore → 源码 → 对应文档 | docs/*.md |
...
**全项目所有 Markdown 文档(包括 `docs/`、`openspec/specs/`、`.claude/docs/`、`README.md`、`CLAUDE.md`、`SNAPSHOT.md`、`tasks.md`、PR description、commit message 等)严禁使用 emoji。**

- 状态用文字代替(`(已完成)` / `(规划中)` / `(待启动)` / `(违规)` / `(不通过)` 等)
- 章节标题不带任何符号装饰(标题只用文字,如 `## 这是什么`)
- 列表项不用符号装饰起始(用 `- ` 或 `* `)
- 表格单元格不放装饰符号(矩阵命中用 `是` / `否`,状态用 `(已完成)` 等文字)
- 不允许任何表情符号(笑脸、庆祝、星形、火焰、闪光、箭头图标等),包括但不限于 Unicode `U+1F300-1FAFF`、`U+2600-27BF`(symbols/dingbats)、变体选择符 `U+FE0F`
...
必须:逐项打勾,每条 step 有输出或显式记录 "N/A(原因)"
...
```
注:关键区分：CodeGraph 有两套调用接口，绝对不能混淆
...
注:误用后果：把 MCP 工具当 bash 命令运行 → "command not found"
...
CodeGraph 可用时（项目已 init，MCP 已连接）：
  ...
  ⭐ 看到 注:stale banner 时，对那一个文件直接 Read，其它继续信任
